  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        {
          status: "error",
          message: "KLING_API_KEY não configurada na Vercel.",
        },
        { status: 500 }
      );
    }
    ...
    const aspectRatio = ["1:1", "16:9", "9:16"].includes(
      body.aspect_ratio
    )
      ? body.aspect_ratio
      : "1:1";
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          status: "error",
          message: "A Kling recusou a solicitação de imagem.",
    ...
    return NextResponse.json({
      status: "success",
      message: "Tarefa de imagem enviada para a Kling.",
      taskId,
      klingResponse: data,
    });
  } catch (error) {
    console.error("Kling Image API error:", error);
